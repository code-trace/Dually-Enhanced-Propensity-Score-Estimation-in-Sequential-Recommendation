...
class DEPSRET(SequentialRecommender):

    input_type = InputType.POINTWISE

    def __init__(self, config, dataset):
        super(DEPSRET, self).__init__(config, dataset)
    ...
    def forward(self, user, item_seq, next_items, user_seq, item_seq_len, user_seq_len):
        ...
        s_n_1s = [
            [
                torch.zeros(self.hidden_size // self.n_heads, self.item_retnet.v_dim // self.n_heads).unsqueeze(0).repeat(item_seq.size(0), 1, 1)
        ...
        item_output = []
        item_s_n_1s=s_n_1s
        for i in range(item_feat_list.size(1)):
            y_n, s_ns = self.item_retnet.forward_recurrent(item_feat_list[:, i:i + 1, :], item_s_n_1s, i)
            item_output.append(y_n)
            item_s_n_1s = s_ns

        item_output = torch.concat(item_output, dim=1)
        ...
        user_output = []
        user_s_n_1s = s_n_1s
        for i in range(user_feat_list.size(1)):
            y_n, s_ns = self.user_retnet.forward_recurrent(user_feat_list[:, i:i + 1, :], user_s_n_1s, i)
            user_output.append(y_n)
            user_s_n_1s = s_ns

        user_output = torch.concat(user_output, dim=1)
