...
class FTRDEPS(SequentialRecommender):

    input_type = InputType.POINTWISE

    def __init__(self, config, dataset):
        super(FTRDEPS, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        ###
        item_output1,target_item_feat_emb,user_output1,target_user_feat_emb,item_seq_emb,user_seq_emb = self.forward(user=user, item_seq=masked_item_seq, next_items=next_items, user_seq=masked_user_seq,
        item_seq_len = item_seq_len, user_seq_len = user_seq_len)
