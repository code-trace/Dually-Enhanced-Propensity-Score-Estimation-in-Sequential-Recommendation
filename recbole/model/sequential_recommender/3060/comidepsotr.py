...
class COMIDEPSOTR(SequentialRecommender):

    input_type = InputType.POINTWISE

    def __init__(self, config, dataset):
        super(COMIDEPSOTR, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        out_r = self.dnn_mlp_layers(multi_interest_emb.view(-1,multi_interest_emb.shape[2]*4))
    ...
    def predict(self, interaction):
        ...
        item_output,target_item_feat_emb,user_output,target_user_feat_emb,item_seq_emb,user_seq_emb = self.forward(user=user, item_seq=item_seq, next_items=test_item, user_seq=user_seq,item_seq_len=item_seq_len,user_seq_len=user_seq_len)
        '''
        ...
        comi_output_dict = self.ComirecSA(item_seq_emb, None, weight_item_emb, target_item_feat_emb,train=False)
        multi_interest_emb = comi_output_dict['multi_interest_emb']
        #in_r = torch.cat([in_r, multi_interest_emb.resize_(in_r.size(0),in_r.size(1))], dim=-1)
        out_r = self.dnn_mlp_layers(multi_interest_emb.resize_(multi_interest_emb.shape[0],multi_interest_emb.shape[1]*4))
        preds = self.dnn_predict_layers(out_r)
...
class MultiInterest_SA(nn.Module):
    ...
    def forward(self, item_seq_emb):
        ...
        mask : batch,seq,1
        '''
        attention_mask = (item_seq_emb > 0).long()  # [B,L]
        #extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)  # torch.int64
...
class ComirecSA(nn.Module):
    ...
    def forward(self, item_seq_emb, target_item,item_emb_weight,target_item_feat_emb,  train=True):

        item_seq_emb = item_seq_emb.detach()
        item_emb_weight = item_emb_weight.detach()
        #item_seq_emb = torch.matmul(item_seq_emb, item_emb_weight.transpose(0, 1))
        if train:
            multi_interest_emb = self.multi_interest_layer(item_seq_emb)  # Batch,K,Emb
            cos_res = torch.bmm(multi_interest_emb, target_item_feat_emb.squeeze(1).unsqueeze(-1))
            k_index = torch.max(cos_res, dim=1)[1]
            best_interest_emb = torch.rand((multi_interest_emb.shape[0], multi_interest_emb.shape[2]),device=multi_interest_emb.device)
