...
class PropensityEstimation(SequentialRecommender):
    def __init__(self, config, dataset):
        super(PropensityEstimation, self).__init__(config, dataset)
        ...
        self.IPS_item_encoder = TransformerEncoder(
        ...
        self.IPS_user_encoder = TransformerEncoder(
            n_layers=self.n_layers,
            n_heads=self.n_heads,
            hidden_size=self.hidden_size,
            inner_size=self.inner_size,
    ...
    def forward(self, user, input_emb_item, next_items, user_seq, item_seq_len, user_seq_len):
...
class FDEPS(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.PropensityEstimation = PropensityEstimation(config,dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        ###
        item_output1,target_item_feat_emb,user_output1,target_user_feat_emb,item_seq_emb,user_seq_emb = self.PropensityEstimation(user=user, item_seq=masked_item_seq, next_items=next_items, user_seq=masked_user_seq,
        item_seq_len = item_seq_len, user_seq_len = user_seq_len)
        itemseq_output = self.gather_indexes(item_output1, torch.max(torch.zeros_like(item_seq_len), item_seq_len - 1))
        ...
        userseq_output = self.gather_indexes(user_output1, torch.max(torch.zeros_like(user_seq_len), user_seq_len - 1))
