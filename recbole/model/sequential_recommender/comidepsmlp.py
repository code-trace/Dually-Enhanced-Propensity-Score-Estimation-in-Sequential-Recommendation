...
class COMIDEPSMLP(SequentialRecommender):
    input_type = InputType.POINTWISE

    def __init__(self, config, dataset):
        super(COMIDEPSMLP, self).__init__(config, dataset)
        ...
        self.UserIPS_Estimation_Net = Timeware_Propensity_Estimation(hidden_size=self.hidden_size,
                                                                     dropout_prob=self.dropout_prob)
        self.itemComirecSA = ComirecSA(self.hidden_size, self.interest_num)
        self.userComirecSA = ComirecSA(self.hidden_size, self.interest_num)
        ...
        if self.training:
            ...
            if self.current_step > self.warmup_rate * self.total_step:
                ...
                if self.current_step % (self.batch_step * (1 + self.IPS_training_rate)) <= self.batch_step:
                    return loss*20 + itemcomi_loss+ usercomi_loss
                else:
                    return loss*20 + itemcomi_loss+ usercomi_loss
            else:
