...
class COMIDEPSQ(SequentialRecommender):

    input_type = InputType.POINTWISE

    def __init__(self, config, dataset):
        super(COMIDEPSQ, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        target_feat_emb = torch.cat((target_item_feat_emb, target_user_feat_emb), dim=-1)
        in_r = torch.cat([user_emb, item_emb], dim=-1)
        comi_output_dict = self.ComirecSA(in_r, next_items, test_item_emb, target_feat_emb,masked_item_seq)
        ...
        out_r = self.dnn_mlp_layers(multi_interest_emb)
    ...
    def predict(self, interaction):
        ...
        comi_output_dict = self.ComirecSA(in_r, None, weight_item_emb, target_item_feat_emb,item_seq,train=False)
        multi_interest_emb = comi_output_dict['multi_interest_emb']
...
class ComirecSA(nn.Module):
    ...
    def forward(self, item_seq_emb, target_item,item_emb_weight,target_item_feat_emb, masked_item_seq, train=True):
        ...
        cos_res = torch.bmm(multi_interest_emb, target_item_feat_emb)
        k_index = torch.max(multi_interest_emb, dim=0)[1]
        best_interest_emb = torch.rand(( multi_interest_emb.shape[1]),
                                       device=multi_interest_emb.device)

        best_interest_emb= multi_interest_emb[k_index,: ]
        if train:
