...
class PropensityEstimation(nn.Module):
    ...
    def forward(self,item_seq_emb, target_item,item_emb_weight,masked_item_seq):
        ...
        logits = torch.matmul(item_output*((masked_item_seq>0).view(masked_item_seq.size(0),-1,1)), item_emb_weight.transpose(0, 1))
...
class FDEPS(SequentialRecommender):

    input_type = InputType.POINTWISE

    def __init__(self, config, dataset):
        super(FDEPS, self).__init__(config, dataset)
