...
class Timeware_Propensity_Estimation(nn.Module):
    ...
    def forward(self,item_seq_emb, target_item, item_emb, seq_len,item_seq):
        ...
        gru_output=gru_output+extended_attention_mask
        ipsleanr=torch.softmax(self.linear1(gru_output),1)
...
class DEPSADDIPS(SequentialRecommender):

    input_type = InputType.POINTWISE

    def __init__(self, config, dataset):
        super(DEPSADDIPS, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        p_item_score, item_IPS_loss, item_logits = self.ItemIPS_Estimation_Net(item_seq_emb,next_items,test_item_emb,item_seq_len,item_seq)
        p_user_score, user_IPS_loss, user_logits = self.UserIPS_Estimation_Net(user_seq_emb,user,test_user_emb,user_seq_len,user_seq)
