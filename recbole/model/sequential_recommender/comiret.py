...
class COMIRET(SequentialRecommender):
    input_type = InputType.POINTWISE

    def __init__(self, config, dataset):
        super(COMIRET, self).__init__(config, dataset)
        ...
        self.UserIPS_Estimation_Net = Timeware_Propensity_Estimation(hidden_size=self.hidden_size,
                                                                     dropout_prob=self.dropout_prob)
        self.itemComirecSA = ComirecSA(self.hidden_size, self.interest_num,self.n_items + 1)
        self.userComirecSA = ComirecSA(self.hidden_size, self.interest_num,self.n_users + 1)
    ...
    def calculate_loss(self, interaction):
        ...
        itemcomi_output = self.itemComirecSA(item_seq_emb, next_items, test_item_emb, target_item_feat_emb,masked_item_seq)
        itemcomi_loss = itemcomi_output['loss']
        itemmulti_interest_emb = itemcomi_output['multi_interest_emb']
        usercomi_output = self.userComirecSA(user_seq_emb, user, test_user_emb, target_user_feat_emb,masked_user_seq)
        usercomi_loss = usercomi_output['loss']
        usermulti_interest_emb = usercomi_output['multi_interest_emb']
        in_r = torch.cat([in_r, itemmulti_interest_emb,usermulti_interest_emb], dim=-1)
        ...
        loss = F.binary_cross_entropy(cos_res, label,  reduction='mean')
        #loss = self.loss(cos_res, label, p_item_score, p_user_score)
        print(f"usercomi_loss:{usercomi_loss},itemcomi_loss:{itemcomi_loss},loss:{loss}")
        if self.training:
            ...
            if self.current_step > self.warmup_rate * self.total_step:
                ...
                if self.current_step % (self.batch_step * (1 + self.IPS_training_rate)) <= self.batch_step:
                    return loss*10 + itemcomi_loss+ usercomi_loss
                else:
                    return loss*10 + itemcomi_loss+ usercomi_loss
            else:
                return item_mlm_loss + user_mlm_loss  + itemcomi_loss+ usercomi_loss
    ...
    def predict(self, interaction):
        ...
        itemcomi_output_dict = self.itemComirecSA(item_seq_emb, None, weight_item_emb, target_item_feat_emb, item_seq,
                                          train=False)
        itemmulti_interest_emb = itemcomi_output_dict['multi_interest_emb']
        weight_user_emb = self.user_embedding.weight[:self.n_users]
        usercomi_output_dict = self.userComirecSA(user_seq_emb, None, weight_user_emb, target_user_feat_emb, user_seq,
                                                  train=False)
        usermulti_interest_emb =usercomi_output_dict['multi_interest_emb']
        in_r = torch.cat([in_r, itemmulti_interest_emb,usermulti_interest_emb], dim=-1)
        cos_res = self.interest_SA(in_r, target_item_feat_emb)
...
class MultiInterest_SA(nn.Module):
    ...
    def forward(self, item_seq_emb, masked_item_seq):
        ...
        A = self.linear2(output) + extended_attention_mask
...
class ComirecSA(nn.Module):
    def __init__(self, hidden_size, interest_num,num_embedding):
        ...
        self.itemseq_embedding = nn.Embedding(self.num_embedding, hidden_size, padding_idx=0)  # mask token add 1
    ...
    def forward(self, item_seq_emb, target_item, item_emb_weight, target_item_feat_emb, masked_item_seq, train=True):
        ...
        item_seq_emb = self.dropout(self.LayerNorm(self.itemseq_embedding(masked_item_seq)))
        weight_item_emb = self.itemseq_embedding.weight[:self.num_embedding]
        item_emb_weight = weight_item_emb
...
class Interest_SA(nn.Module):
    def __init__(self, hidden_size, interest_num, d=None):
        ...
        self.linear1 = nn.Linear(self.hidden_size, self.d)
        self.linear2 = nn.Linear(self.d, 6)
